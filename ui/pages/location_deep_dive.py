...
def render(master_df, profitability_model, format_recommender):
    ...
    with c1:
        fig = go.Figure(data=go.Scatterpolar(
            ...
            line=dict(color="#1B7A42", width=2),
            marker=dict(size=6, color="#1B7A42"),
        ))
        fig.update_layout(
    ...
    fig2.add_trace(go.Scatter(x=years, y=[f["fuel_revenue_cr"] for f in flows],
                              name="Fuel", stackgroup="one", line=dict(width=0), fillcolor="rgba(27,94,59,0.6)"))
    fig2.add_trace(go.Scatter(x=years, y=[f["ev_revenue_cr"] for f in flows],
                              name="EV Charging", stackgroup="one", line=dict(width=0), fillcolor="rgba(34,197,94,0.6)"))
    ...
    fig3.update_layout(
        ...
        margin=dict(t=40, b=30), paper_bgcolor="#FAFDF9", plot_bgcolor="#FAFDF9",
        font=dict(family="Inter"),
    )
